...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en">
      <body style={{ background: "#ede9ff", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        <Navbar />
        <main style={{ maxWidth: "1100px", width: "100%", margin: "0 auto", padding: "0 16px 64px", flex: 1 }}>
          {children}
        </main>
        <footer style={{ borderTop: "1px solid rgba(109,40,217,0.12)", padding: "24px 16px", textAlign: "center", background: "#e4deff" }}>
          <div style={{ maxWidth: "1100px", margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: "16px" }}>
            ...
            <span style={{ fontSize: "12px", color: "rgba(109,40,217,0.3)" }}>·</span>
            <Link href="/privacy" style={{ fontSize: "12px", color: "#6d60c0", textDecoration: "none", fontWeight: 500 }}>Privacy Policy</Link>
            <Link href="/terms" style={{ fontSize: "12px", color: "#6d60c0", textDecoration: "none", fontWeight: 500 }}>Terms of Service</Link>
            <span style={{ fontSize: "12px", color: "rgba(109,40,217,0.3)" }}>·</span>
            <span style={{ fontSize: "12px", color: "#6d60c0" }}>© {new Date().getFullYear()} IndieScout</span>
          </div>
        </footer>
      </body>
    </html>
