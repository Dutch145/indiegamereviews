              ...
              return (
                <Link key={review.id} href={`/games/${review.games?.slug}`} style={{ textDecoration: "none", display: "flex", gap: "16px", background: "rgba(109,40,217,0.03)", border: "1px solid rgba(109,40,217,0.08)", borderRadius: "12px", padding: "16px", alignItems: "stretch" }}>
                  <div style={{ width: "64px", flexShrink: 0, borderRadius: "8px", background: "rgba(109,40,217,0.08)", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", minHeight: "86px" }}>
                    {review.games?.cover_url ? <img src={review.games.cover_url} alt={review.games.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ color: "#7c3aed", fontWeight: 700, fontSize: "18px" }}>{review.games?.title[0]}</span>}
                  </div>
                  <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                    <div>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "10px", marginBottom: "4px" }}>
                        <p style={{ fontSize: "15px", fontWeight: 700, color: "#1e1b4b" }}>{review.games?.title}</p>
                        <span style={{ fontSize: "14px", fontWeight: 800, color: sc.color, background: sc.bg, padding: "4px 10px", borderRadius: "8px", flexShrink: 0 }}>{review.score_overall}</span>
                      </div>
                      <p style={{ fontSize: "13px", color: "#6d60c0", lineHeight: 1.6, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", marginBottom: "8px" } as React.CSSProperties}>{review.verdict}</p>
                    </div>
                    <p style={{ fontSize: "11px", color: "#9d8fc0" }}>
                      By <span style={{ fontWeight: 600, color: "#6d60c0" }}>{review.author}</span> · {formatDate(review.published_at)}
                    </p>
                  </div>
                </Link>
              )
