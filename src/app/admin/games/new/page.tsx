    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <Field label="Title *"><input className={inputClass} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Hollow Depths" /></Field>
          <Field label="Slug (auto-generated)"><input className={`${inputClass} bg-gray-50 text-gray-400`} value={slug} readOnly /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Developer *"><input className={inputClass} value={developer} onChange={(e) => setDeveloper(e.target.value)} /></Field>
          <Field label="Publisher"><input className={inputClass} value={publisher} onChange={(e) => setPublisher(e.target.value)} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Release year"><input className={inputClass} type="number" value={releaseYear} onChange={(e) => setReleaseYear(e.target.value)} placeholder="2024" /></Field>
          <Field label="Genres (comma-separated)"><input className={inputClass} value={genres} onChange={(e) => setGenres(e.target.value)} placeholder="Roguelike, Action" /></Field>
        </div>
        <Field label="Platforms (comma-separated)"><input className={inputClass} value={platforms} onChange={(e) => setPlatforms(e.target.value)} placeholder="PC, Nintendo Switch" /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Cover image URL"><input className={inputClass} value={coverUrl} onChange={(e) => setCoverUrl(e.target.value)} placeholder="https://..." /></Field>
          <Field label="Banner image URL"><input className={inputClass} value={bannerUrl} onChange={(e) => setBannerUrl(e.target.value)} placeholder="https://..." /></Field>
        </div>
        <div className="border-t border-gray-100 pt-5 space-y-4">
          ...
          {isSpotlight && (
            <Field label="Spotlight quote (optional)">
              <textarea className={textareaClass} rows={2} value={spotlightQuote} onChange={(e) => setSpotlightQuote(e.target.value)} placeholder="A short editorial quote..." />
            </Field>
          )}
