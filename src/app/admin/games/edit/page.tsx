  ...
  useEffect(() => {
    ...
    supabase.from("games").select("*").eq("id", id).single().then(({ data }) => {
      if (data) {
        ...
        setReleaseYear(g.release_year?.toString() ?? "")
        setGenres((g.genres ?? []).join(", "))
        setPlatforms((g.platforms ?? []).join(", "))
        ...
        setEditorPickLabel(g.editor_pick_label ?? "")
      }
      setFetching(false)
    })
  }, [id])
    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Slug (read-only)"><input className={`${inputClass} bg-gray-50 text-gray-400`} value={slug} readOnly /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Release year"><input className={inputClass} type="number" value={releaseYear} onChange={(e) => setReleaseYear(e.target.value)} /></Field>
          <Field label="Genres (comma-separated)"><input className={inputClass} value={genres} onChange={(e) => setGenres(e.target.value)} /></Field>
        </div>
        <Field label="Platforms (comma-separated)"><input className={inputClass} value={platforms} onChange={(e) => setPlatforms(e.target.value)} /></Field>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <button onClick={handleDelete} className="text-sm text-red-500 hover:text-red-700 transition-colors">Delete game</button>
        </div>
      </div>
    </div>
  )
